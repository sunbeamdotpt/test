...
impl Headscale {
    ...
    pub async fn start(self) -> Result<ContainerAsync<GenericImage>, testcontainers::TestcontainersError> {
        ...
        util::build_image(&descriptor, &dockerfile, &[("config.yaml", self.config.as_bytes())])
            .await
            .map_err(|e| testcontainers::TestcontainersError::other(format!("build headscale image: {e}")))?;
